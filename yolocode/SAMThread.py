...
class SAMPredictorThread(YOLOv8SegThread):
    ...
    def generate(
        ...
        for crop_region, layer_idx in zip(crop_regions, layer_idxs):
            ...
            keep = torchvision.ops.nms(crop_bboxes, crop_scores, self.iou_thres)  # NMS
    ...
    def get_model(self):
        ...
        from ultralytics.models.sam.build import build_sam  # slow import
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        for masks, orig_img, img_path in zip([pred_masks], orig_imgs, self.batch[0]):
            if masks.shape[0] == 0:
                ...
                idx = pred_scores > self.conf_thres
    ...
    def inference_features(
        ...
        """
        dst_shape = dst_shape or (self.imgsz, self.imgsz)
...
class SAM2DynamicInteractivePredictorThread(SAM2PredictorThread):
    ...
    def get_im_features(self, img: torch.Tensor | np.ndarray) -> None:
        ...
        """
        vis_feats, vis_pos_embed, feat_sizes = SAM2VideoPredictorThread.get_im_features(self, img, batch=self._max_obj_num)
...
class SAM3PredictorThread(SAM2PredictorThread):
    ...
    def get_model(self):
        ...
        from ultralytics.models.sam.build_sam3 import build_interactive_sam3  # slow import

        return build_interactive_sam3(self.model, compile=self.compile)


class SAM3SemanticPredictorThread(SAM3PredictorThread):
    """Segment Anything Model 3 (SAM3) Predictor for image segmentation tasks."""

    def get_model(self):
        """Retrieve and initialize the Segment Anything Model 3 (SAM3) for image segmentation tasks."""
        from ultralytics.models.sam.build_sam3 import build_sam3_image_model  # slow import

        return build_sam3_image_model(self.model, compile=self.compile)
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        keep = pred_scores > self.conf_thres
...
class SAM3VideoPredictorThread(SAM2VideoPredictorThread, SAM3PredictorThread):
...
class SAM3VideoSemanticPredictorThread(SAM3SemanticPredictorThread):
    ...
    def __init__(
        ...
        # build SAM3 tracker
        self.tracker = SAM3VideoPredictorThread(overrides=overrides)
    ...
    def setup_model(self, model=None, verbose=True):
        ...
        model = build_interactive_sam3(self.model, with_backbone=False)
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        if len(curr_obj_ids) == 0:
            ...
            keep = (pred_scores > self.conf_thres) & pred_masks.any(dim=(1, 2))
            pred_masks = pred_masks[keep]
