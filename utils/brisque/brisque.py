...
import os
from utils.brisque.models import MODEL_PATH
...
class BRISQUE:
    ...
    def gen_svm_nodearray(self, xi, feature_max=None, isKernel=False):
        if feature_max:
            assert (isinstance(feature_max, int))

        xi_shift = 0  # ensure correct indices of xi
        if np and isinstance(xi, tuple) and len(xi) == 2 \
        ...
        elif np and isinstance(xi, np.ndarray):
            if not isKernel:
                xi_shift = 1
        ...
        ret = (svmutil.svm_node * (len(index_range) + 1))()
        ret[-1].index = -1
